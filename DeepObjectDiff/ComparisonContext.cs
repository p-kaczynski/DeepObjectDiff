...
{
    /// <summary>
    ///     This object represents a context that is passed down during traversal of the object graph to keep track of things
    ///     like where in the graph we are,
    ///     which objects were visited and so on. It is clonable, because at some points during object graph traversal we need
    ///     to ensure that we don't mess up the stacks
    ...
    internal class ComparisonContext : ICloneable
    {
        ...
        /// </summary>
        private ComparisonContext(Stack<string> stack, List<ObjectDifference> differences,
            Stack<WeakReference> firstObjectPath, Stack<WeakReference> secondObjectPath)
        {
            _stack = stack;
        ...
        /// <summary>
        ///     Allows preventing generating differences. Used when <see cref="ObjectComparer.Compare{T}" /> is used to compare
        ///     elements of a collection (it would generate
        ...
        /// <summary>
        ///     Clones the <see cref="ComparisonContext" /> in a predefined way to allow for independent traversal of different
        ///     paths of the object graph
        /// </summary>
        /// <returns></returns>
        public object Clone() => new ComparisonContext(new Stack<string>(_stack), _differences,
            new Stack<WeakReference>(_firstObjectPath), new Stack<WeakReference>(_secondObjectPath));

        /// <summary>
        ///     Remembers a name of an object graph node the comparison has entered, as inside the object it is impossible to find
        ///     out the name of field/property whence we've came
        /// </summary>
        /// <param name="name"></param>
        internal void Enter(string name) => _stack.Push(name);

        /// <summary>
        ///     Tries to note entering a particular reference-type object.
        ///     It will return false, if in current graph traverse path we have alredy entered this particular object, which means
        ///     there is a circular reference, and code would never finish
        /// </summary>
        ...
        /// <returns>
        ///     <c>true</c> if the object is a <see cref="string" />, a <see cref="ValueType" /> or if this is the first time
        ///     in current recursion path the object has been entered, otherwise <c>false</c>
        /// </returns>
        /// <remarks>
        ///     This uses <see cref="WeakReference" /> to be extra-sure we won't cause GC to prevent collection of compared
        ///     objects, if by some means this context persists longer than the compared objects.
        /// </remarks>
        ...
        /// <summary>
        ///     Informs the context that an object comparison has been finished, and <see cref="ObjectComparer.Compare{T}" /> will
        ///     return.
        ///     This removes information about having entered last object and will not cause <c>false</c> return from
        ///     <see cref="TryEnterObject{TRef}" /> if the object is entered again in a different execution path
        /// </summary>
        ...
        /// <summary>
        ///     Forgets the (field/property) name of last entered object, as the differences found from now on will not be in that
        ///     particular object
        /// </summary>
        internal void Exit() => _stack.Pop();
        ...
        /// <inheritdoc />
        /// <summary>
        ///     A simple comparer of <see cref="T:System.WeakReference" />s to determine whether two WeakReferences are pointing to
        ///     the same object instance
        /// </summary>
        private class ReferenceEqualityComparer : IEqualityComparer<WeakReference>
        {
            /// <summary>
            ///     Provides a single instance of the <see cref="ReferenceEqualityComparer" /> to be reused. There is no need for it to
