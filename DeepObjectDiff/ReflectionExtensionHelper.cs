...
namespace DeepObjectDiff
{
    ...
    {
        /// <summary>
        /// Checks if the type <paramref name="type"/> implements <see cref="IDictionary{TKey,TValue}"/>, and if so returns key and value types
        /// </summary>
        ...
        /// <returns><c>true</c> if <paramref name="type"/> implements <see cref="IDictionary{TKey,TValue}"/>, otherwise <c>false</c></returns>
        ...
        /// <summary>
        /// CHecks if the type <paramref name="type"/> implements <see cref="IEnumerable{T}"/>, and if so returns element type
        /// </summary>
        ...
        /// <summary>
        /// Helper to check if <paramref name="type"/> implements <paramref name="genericInterface"/> and if so, returns its <paramref name="elementType"/>
        /// </summary>
        /// <param name="type">Type to check if implements <paramref name="genericInterface"/></param>
        /// <param name="genericInterface"></param>
        /// <param name="elementType">If <see cref="type"/> implements <paramref name="genericInterface"/> returns its element type (i.e. its first generic argument)</param>
        /// <returns><c>true</c> if <paramref name="type"/> implements <paramref name="genericInterface"/>, otherwise <c>false</c></returns>
        /// <remarks>
