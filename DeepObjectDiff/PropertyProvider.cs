...
namespace DeepObjectDiff
{
    ...
    internal class PropertyProvider
    {
        ...
        /// <returns>If exists, <see cref="PropertyInfo"/> of a property by name of <paramref name="name"/> in type <typeparamref name="T"/></returns>
        [CanBeNull]
        internal PropertyInfo GetProperty<T>(string name)
            => _cache.GetOrAdd(typeof(T), CacheType)
        ...
        /// <returns>A dictionary of <see cref="MemberInfo.Name"/> to <see cref="PropertyInfo"/> of selected properties of type <paramref name="type"/></returns>
        [NotNull]
